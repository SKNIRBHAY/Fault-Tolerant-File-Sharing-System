...
//------------------------------------UDP to Intermediate Server End-------------------------------------------	
	...
	//printf("sendBuff : %s shubh check \n",sendBuff);

	write(conn_fd, sendBuff, strlen(sendBuff));	
	/*n = read(conn_fd, recvBuff, sizeof(recvBuff));	
	recvBuff[n]='\0';
	//printf("\n%s shubh check for authorisation \n",recvBuff);
	if (recvBuff=="Not Authorised")
	{
		close(conn_fd);	
		return 0;
	}
	//else if (recvBuff == "Authorised")
			
	printf("\n%s shubh check for req \n",recvBuff);*/
		//while(1)
		//{
