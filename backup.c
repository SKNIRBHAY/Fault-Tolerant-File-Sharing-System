...
void *synchronise(void *arg){
	...
	backup_addr.sin_port = htons(PORTNO_MAIN);

	if(inet_pton(AF_INET, IP_MAIN, &backup_addr.sin_addr)== -1){
...
int handleUpload(int connfd,char *filename){
	...
	strcpy(filepath,"./backup_repository/"); /*appending root directory path*/
...
char* authorise(char buf[]){
	char s[2] = " ";
      ...
      if(strcmp(tokens[0],users[0])==0)
   	...
   	printf(" #### SHUBH CHECK HERE 3  ####\n");
   	printf(" #### tokens[0] :%s,users[0] :%s,\n",tokens[0],users[0]);
   	printf(" #### tokens[1] :%s,users[1] :%s,\n",tokens[1],users[1]);
   	...
   	if(strcmp(tokens[1],users[1])==0)
   		printf("\n password is correct");
   	//testing complete
   	
   	if(strcmp(tokens[0],users[0])==0 && strcmp(tokens[1],users[1])==0){
   		printf(" #### SHUBH CHECK HERE 4  ####\n");
   		return "Authorised";
   	}
   	else{ 
   		printf(" #### SHUBH CHECK HERE 5  ####\n");
   		return "Not Authorised";
   	}
}
...
int main(int argc,char **argv){
	...
	my_addr.sin_port = htons(PORTNO_BACKUP);
